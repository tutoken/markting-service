...
public class TimeUtil {
    ...
    public static void MARK(String tag, String step) {
        if (timestampMap.containsKey(tag)) {
            ...
            logger.info(tag + ": " + step + ", time elapse: " + MONITOR(t - LAST_TIMESTAMP));
            LAST_TIMESTAMP = t;
        } else {
            LAST_TIMESTAMP = System.currentTimeMillis();
            timestampMap.put(tag, LAST_TIMESTAMP);
            logger.info(tag + " begins at " + GET_DATE_BY_STAMP(LAST_TIMESTAMP));
        }
    }

    public static void END(String tag) {
        logger.info(tag + " end at " + GET_DATE_BY_STAMP(System.currentTimeMillis()) + ", time elapse " + MONITOR(System.currentTimeMillis() - timestampMap.get(tag)));
        timestampMap.clear();
    }
