...
public class TronChainService extends DefaultChainService {
    ...
    public int getTransactionCount() {
        ...
        try {
            String url = getBlockchainScanURL() + "token_trc20/transfers?"
                    + requiredParam("limit", "10000")
                    + requiredParam("start", "10000")
                    + requiredParam("contract_address", token.getContract(this.chain))
    ...
    public JSONArray getTransactionsByAddress(QueryParam queryParam) {
        String url = getBlockchainScanURL() + "transaction?=account"
                + requiredParam("action", "txlist")
                + requiredParam("address", queryParam.getAddress())
                + requiredParam("start_timestamp", queryParam.getStartTime())
                + requiredParam("end_timestamp", queryParam.getEndTime());
    ...
    public Map<String, Map<String, String>> getTransactionsByEvent(QueryParam queryParam) {
        String requestUri = getJsonRpcURL() + "v1/contracts/" + queryParam.getAddress()
    ...
    public JSONArray getERC20Transactions(QueryParam param) {
        ...
        String url = getBlockchainScanURL() + "token_trc20/transfers?module=account" +
                optionalParam("limit", param.getOffset()) +
                optionalParam("relatedAddress", param.getAddress()) +
                requiredParam("contract_address", param.getContractAddress()) +
                optionalParam("start_timestamp", param.getStartBlock()) +
                optionalParam("end_timestamp", param.getEndBlock()) +
                optionalParam("apikey", getApiKey());

        JSONObject response = JSON.parseObject(HttpUtil.get(url));
        return response.getJSONArray("token_transfers");
    }
}
